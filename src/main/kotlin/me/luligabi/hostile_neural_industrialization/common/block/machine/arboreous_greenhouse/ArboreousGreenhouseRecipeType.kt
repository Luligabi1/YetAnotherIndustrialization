...
import me.luligabi.hostile_neural_industrialization.common.item.ChaChaRealSmoothItem
...
import me.luligabi.hostile_neural_industrialization.mixin.MIRecipeJsonAccessor
...
class ArboreousGreenhouseRecipeType(id: ResourceLocation): ProxyableMachineRecipeType(id) {
    ...
    private fun generate(
        ...
        val regularRecipe = generateBase(data, tier, sapling, level)?.let {
            RecipeHolder(HNI.id(id), it.convert() as MachineRecipe)
        } ?: return null

        val nutrientRecipe = generateBase(data, tier, sapling, level, true)?.let {
            RecipeHolder(HNI.id("$id/nutrient"), it.convert() as MachineRecipe)
        } ?: return null
